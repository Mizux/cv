...
\item managemed a contractor for an UVC compliant firmware (CMake, Docker, C++, Catch, Plantuml).
\end{itemize}
Vision Team:
\begin{itemize}
\item reworked and maintained a C++ Framework for multi-client access to robot cameras (CMake, C++, Boost).
\item developed camera viewer tooling (C++, Qt).
\item developed Modularity: a C++ computational graph framework for perception.
\item maintained the internal CI Builfarm and Training (Jenkins,
 gcovr).
...
\item developed of debugger tools (C++, Qt).
\end{itemize}
% Software
